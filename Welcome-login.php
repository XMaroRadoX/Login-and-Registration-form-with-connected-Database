            ...
            <div class="form-box p-md-5 p-3" id="login-form">
                <div class="form-title">
                    <h2 class="fw-bold mb-3">
                        ...
                        $passwordLogin = md5($_POST['password-login']);

                        $query = "SELECT * FROM user WHERE email = '$emailLogin' and password = '$passwordLogin'";
                        $result = mysqli_query($mysqli, $query);
                        if (mysqli_num_rows($result) > 0) {
                            $name = mysqli_fetch_row($result)[2];
                            echo "Welcomeeeee $name";
                        } else {
                            header('index.php?errorLogin=yes');
